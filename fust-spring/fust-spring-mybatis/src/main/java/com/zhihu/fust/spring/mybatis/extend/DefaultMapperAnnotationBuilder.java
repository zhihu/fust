...
import com.zhihu.fust.spring.mybatis.operations.BatchCreateOp;
import com.zhihu.fust.spring.mybatis.operations.BatchPatchOp;
import com.zhihu.fust.spring.mybatis.operations.CreateOp;
import com.zhihu.fust.spring.mybatis.operations.FindOp;
import com.zhihu.fust.spring.mybatis.operations.Operation;
import com.zhihu.fust.spring.mybatis.operations.PatchOp;
import com.zhihu.fust.spring.mybatis.operations.RemoveOp;
import com.zhihu.fust.spring.mybatis.operations.UpdateOp;
import com.zhihu.fust.spring.mybatis.util.ClassUtils;

/**
 * 继承父类，由于大部分方法都是私有方法，只能复写
 */
public class DefaultMapperAnnotationBuilder extends MapperAnnotationBuilder {
    ...
    public DefaultMapperAnnotationBuilder(Configuration configuration, Class<?> type) {
        ...
        sqlAnnotationTypes.add(org.apache.ibatis.annotations.Select.class);
    ...
    public void parse() {
        ...
        if (!configuration.isResourceLoaded(resource)) {
            ...
            // gin
            if (TemplateDao.class.isAssignableFrom(type)) {
                injectTemplateDao(assistant, type);
            }
            ...
            for (Method method : methods) {
                try {
                    ...
                    configuration.addIncompleteMethod(new DefaultMethodResolver(this, method));
                }
            }
        }
        parsePendingMethods();
        ...
        if (!configuration.isResourceLoaded("namespace:" + type.getName())) {
            ...
            if (inputStream != null) {
                XMLMapperBuilder xmlParser = new XMLMapperBuilder(inputStream, assistant.getConfiguration(),
                        xmlResource, configuration.getSqlFragments(),
                        type.getName());
                xmlParser.parse();
            }
        }
    ...
    private SqlCommandType getSqlCommandType(Method method) {
        Class<? extends Annotation> annoType = getSqlAnnotationType(method);

        if (annoType == null) {
            annoType = getSqlProviderAnnotationType(method);

            if (annoType == null) {
                return SqlCommandType.UNKNOWN;
            }

            if (annoType == SelectProvider.class) {
                annoType = org.apache.ibatis.annotations.Select.class;
            } else if (annoType == InsertProvider.class) {
                annoType = Insert.class;
            } else if (annoType == UpdateProvider.class) {
                annoType = Update.class;
            } else if (annoType == DeleteProvider.class) {
                annoType = Delete.class;
            }
        }

        return SqlCommandType.valueOf(annoType.getSimpleName().toUpperCase(Locale.ENGLISH));
    }
    ...
    private Class<? extends Annotation> chooseAnnotationType(Method method,
        ...
        for (Class<? extends Annotation> annoType : types) {
            Annotation annotation = method.getAnnotation(annoType);
            if (annotation != null) {
                return annoType;
            }
        }
    ...
    private static Map<Configuration, Set<String>> mapperRegistryCache = new HashMap<>();

    private static List<Operation> listOf() {
        return Stream.of(
                new FindOp(),
                new CreateOp(),
                new UpdateOp(),
    ...
    private static Set<String> getMapperRegistryCache(Configuration configuration) {
        Set<String> ret = mapperRegistryCache.get(configuration);
        if (ret == null) {
            ret = new HashSet<>();
        }
        return ret;
    ...
    private void injectTemplateDao(MapperBuilderAssistant builderAssistant, Class<?> mapperClass) {
        if (TemplateDao.class.isAssignableFrom(mapperClass)) {

            Class<?> entityClass = ClassUtils.extractModelClass(mapperClass);
            Objects.requireNonNull(entityClass, "extractModelClass error, entityClass is null");

            // register typeHandler
            DefaultTypeHandlerRegistry.registerGinTypeHandlers(mapperClass, entityClass, builderAssistant.getConfiguration());

            // register resultMap
            DefaultResultMapRegister.registerResultMap(mapperClass, entityClass,
                    builderAssistant.getConfiguration());

            String className = mapperClass.toString();
            Set<String> registryCache = getMapperRegistryCache(builderAssistant.getConfiguration());
            if (!registryCache.contains(className)) {
                List<Operation> methodList = listOf();
                // 循环注入自定义方法
                methodList.forEach(m -> m.bind(builderAssistant, mapperClass));
                registryCache.add(className);
            }
